...
resourcename="flinstones"  # lower case, only
resourcegroup="flinstones_rg"
...
# git clone https://github.com/apilogicserver/flinstones.git
# cd flinstones
...
az group create --name flinstones_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group flinstones_rg --sku S1 --is-linux
az container create --resource-group flinstones_rg --name flinstones --image apilogicserver/flinstones:latest --dns-name-label flinstones --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//flinstones.westus.azurecontainer.io:5656'
